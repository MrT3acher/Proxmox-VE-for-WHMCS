...
if (isset($_GET['pveticket']) && isset($_GET['host']) && isset($_GET['path']) && isset($_GET['vncticket'])) {
	...
	setrawcookie('PVEAuthCookie', $pveticket, 0, '/');

	// Create the final noVNC URL with the re-encoded vncticket
	$hostname = $host;
	$redirect_url = '/modules/servers/pvewhmcs/novnc/vnc.html?autoconnect=true&encrypt=true&host=' . $hostname . '&port=443&password=' . urlencode($vncticket) . '&path=' . urlencode($path);
